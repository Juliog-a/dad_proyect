...
public class RestServer extends AbstractVerticle {
	...
	public void start(Promise<Void> startFuture) {
      ...
      router.get("/api/sensor/:id").handler(this::getByIDSen); //ACCEDE BIEN A LOS DATOS PERO NO NOS DA ERROR CON ID INEXISTENTE.
      ...
      router.get("/api/actuador/:id").handler(this::getByIDAct); //ACCEDE BIEN A LOS DATOS PERO NO NOS DA ERROR CON ID INEXISTENTE.
      ...
      router.post("/api/actuador").handler(this::addActuador); //CHECK, BUT HAY QUE MIRAR BIEN EL ACTUADOR_RELE, TENGO DUDAS
      router.delete("/api/actuador/:id").handler(this::deleteActuador); //CHECK, BUT ELIMINAMOS EL SENSOR DE LA PLACA NO EL ACTUADOR EN SI.
...
//	    	}
	        ...
	        mySqlClient.preparedQuery("INSERT INTO sensor (idSensor, nPlaca, humedad, timestamp, temperatura, idGroup) VALUES (?, ?, ?, ?, ?, ?)").execute(
	        		(Tuple.of(sensor.getIdSensor(), sensor.getnPlaca(), sensor.getHumedad(), sensor.getTimestamp(), sensor.getTemperatura(), sensor.getIdGroup())), res -> {
	                if (res.succeeded()) {
		                ...
		                .putHeader("content-type", "application/json; charset=utf-8")
		                        .setStatusCode(404)
		                        .end("Error al añadir los sensores: " + res.cause().getMessage());
	                	}
	            });
	    }
...
//				}
				...
				/////////////////////////////////////////////////////////
				    ...
				    private void addActuador(RoutingContext routingContext) {
				        ...
				        // Ejecutamos la inserción en la base de datos MySQL
				        mySqlClient.preparedQuery("INSERT INTO actuador (nPlaca, idActuador, timestamp) VALUES (?, ?, ?)").execute(
				        		(Tuple.of(actuador.getnPlaca(), actuador.getIdActuador(), actuador.getTimestamp())), res -> {
				                if (res.succeeded()) {
				                    // Si la inserción es exitosa, respondemos con el sensor creado
				                    routingContext.response()
				                        .setStatusCode(201)
