...
String serverName = "http://192.168.183.180:8081";

// Replace WifiName and WifiPassword by your WiFi credentials
#define STASSID "S23"    //"Your_Wifi_SSID"
#define STAPSK "Choco2024." //"Your_Wifi_PASSWORD"
#define DHTPIN 15 // pin de placa del sensor de humedad y temperatura
#define ACTUADOR_PIN 26 // pin de placa del actuador
...
const char *MQTT_BROKER_ADRESS = "192.168.183.180";
...
else if (mensaje=="OFF")
{
  ...
  String actuator_states_body = serializeActuatorStatusBody(1,201,millis(),activo, encendido,5);
...
if (cont == 1000) {
    ...
    if (isnan(event.temperature)) {
      ...
      POST_tests();
    int estadoRele = digitalRead(ACTUADOR_PIN);
    if (estadoRele == HIGH) {
      Serial.println("El relé está activado (HIGH).");
    } else {
      Serial.println("El relé está desactivado (LOW).");
  }
}
else{
  cont++;
      ...
      if (mensaje == "ON") {
        // Activar el relé (poner el pin en HIGH)
        digitalWrite(ACTUADOR_PIN, HIGH);
      } else {
        // Desactivar el relé (poner el pin en LOW)
